    ...
    def __init__(self,state,player):
        ...
        if opponent == "TA":
            ...
            if total_moves<=6:
                ...
                self.max_time = max(5.5,2*current_remaining_time/(len(self.all_moves)))
        elif opponent == "TIE":
            if total_moves<=4:
                self.max_time = 2*current_remaining_time/len((self.all_moves))
            elif total_moves <= 10:
...
class node:
    ...
    def expand(self):
        self.expanded = True
        if opponent=="Group" or opponent=="TIE":
            if total_moves<=6:
...
class AIPlayer:
    ...
    def get_move(self, state: np.array) -> Tuple[int, int]:
        ...
        if(len(all_valid_actions)==1):
            action = all_valid_actions[0]
            return int(action[0]),int(action[1])
